                                         ...
                                         struct ofpbuf *msg)
{
    ...
    if (!ofpfmr) {
        VLOG_WARN_RL(&bad_ofmsg_rl,
                     "OFPMP_FLOW_MONITOR request has %"PRIuSIZE" "
                     "leftover bytes at end", msg->size);
        return OFPERR_OFPFMFC_UNKNOWN;
    }

    flags = ntohs(ofpfmr->flags);
    /* XXX: OFPFMF14_ONLY_OWN is not supported */
    if (!(flags & (OFPFMF14_ADD | OFPFMF14_REMOVED | OFPFMF14_MODIFY))
        || flags & ~(OFPFMF14_INITIAL | OFPFMF14_ADD | OFPFMF14_REMOVED
                     | OFPFMF14_MODIFY | OFPFMF14_INSTRUCTIONS
                     | OFPFMF14_NO_ABBREV)) {
    ...
    /* XXX: Support out_group */
    if (ofpfmr->out_group != htonl(OFPG_ANY)) {
        VLOG_WARN_RL(&bad_ofmsg_rl,
                     "OFPMP_FLOW_MONITOR has bad out_group %#"PRIx32,
                     ntohl(ofpfmr->out_group));
        return OFPERR_OFPMOFC_BAD_OUT;
    }
    rq->table_id = ofpfmr->table_id;
    /* XXX: Support OFPFMC14_MODIFY and OFPFMC14_DELETE */
    if (ofpfmr->command != OFPFMC14_ADD) {
        VLOG_WARN_RL(&bad_ofmsg_rl,
                     "OFPMP_FLOW_MONITOR has bad command %#"PRIx8,
                     ofpfmr->command);
                                    ...
                                    struct ofpbuf *msg)
{
    ...
    if (!msg->size) {
        ...
        return ofputil_decode_of14_flow_monitor_request(rq, msg);
    } else if (raw == OFPRAW_OFPST14_FLOW_MONITOR_REQUEST) {
        return ofputil_decode_nx_flow_monitor_request(rq, msg);
    }
